...
(function () {
  var noBlobConstructorPolicy = function (tip, metoda, origFunkcija) {
      ...
      // ce so podani argumenti preverimo ali je parameter ustrezen
      if (origFunkcija().prototype !== undefined) {
          return function (url) {
...
secure.textContent = secureFN;
secure.async = false;
// console.log(document.documentElement, chrome.extension.getURL('secure.js'));
(document.head || document.documentElement).appendChild(secure);

var s = document.createElement('script');
s.textContent = scriptFn;
s.async = false;
...
secure.onload = function () {
  // secure.parentNode.removeChild(secure);
};
s.onload = function () {
